...
##B        log = e.error_log.filter_from_level(etree.ErrorLevels.FATAL)
...
#    returns: a code (True for valid / False for invalid) and a reason for the code
#    """
#    print ('+++ ENTERING cvrf_validate() ...')
#    try:
#        print ('### Parsing schema ', f)
#        xmlschema_doc = etree.parse(f)
#    except etree.XMLSyntaxError as e:
...
#
#    try:
#        print ('### Validating document ' + cvrf_doc + ' agains schema ' + f)
#        xmlschema.assertValid(cvrf_doc)
#        return True, "Valid"
#    except etree.DocumentInvalid:
#        return False, xmlschema.error_log
#    print ('--- LEAVING cvrf_validate() ...')
...
#    document_title = cvrf_doc.findtext("cvrf:DocumentTitle",namespaces={"cvrf": CVRF_Syntax.NAMESPACES["CVRF"].replace("{", "").replace("}", "")}).strip().replace(" ", "_")
...
def main():
    print ('+++ ENTERING CVRF PARSING STAGE ... ')
...
'''
    for file in os.listdir("CSAs/"):
        if file.endswith(".xml"):
            print ('\n\n###########################################################################')
            print ('### FILE: ', file)
            for event, element in etree.iterparse('CSAs/' + file):
                print ('### TAG: %4s\n### TEXT: %s \n' % (chop_ns_prefix(element.tag), element.text))
'''
#            cvrf_doc = etree.parse('CSAs/' + file, etree.XMLParser(encoding="utf-8"))
#            cvrf_validate('schemas/cvrf.xsd', cvrf_doc)

#    for child in CvrfRoot:
#        print (chop_ns_prefix(child.tag))
#    tree = etree.parse('CSAs/cisco-sa-20160927-openssl.xml')
    
#    print ('--- LEAVING CVRF PARSING STAGE ....')
