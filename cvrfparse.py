...
def print_node(node, strip_ns, f=sys.stdout):
    ...
    """
    if node.tag:
#        print("[%s]" %(chop_ns_prefix(node.tag) if strip_ns else node.tag), end=' ', file=f)
        print ((chop_ns_prefix(node.tag) if strip_ns else node.tag))
        #,end=' ',file=f)
    if node.text:
    ...
    if args.validate is True:
        try:
            if args.schema:
                ...
                print("Fetching schemata...")
                #, file=sys.stderr)
                f = urllib.request.urlopen(CVRF_Syntax.CVRF_SCHEMA)
        except IOError as e:
